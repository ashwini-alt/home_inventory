...
public class InventoryDocument implements Printable {
	...
	if ((pageIndex + 1) > Home_inventory.lastPage)
	{
	return NO_SUCH_PAGE;
	}
	int i, iEnd;
	...
	iEnd =  Home_inventory.entriesPerPage * (pageIndex + 1);
	if (iEnd >  Home_inventory.numberEntries)
	iEnd =  Home_inventory.numberEntries;
	for (i = 0 +  Home_inventory.entriesPerPage * pageIndex; i < iEnd; i++)
	{
	// dividing line
	...
	g2D.drawString( Home_inventory.myInventory[i].description, (int) pf.getImageableX(), y);
	y += dy;
	g2D.setFont(new Font("Arial", Font.PLAIN, 12));
	g2D.drawString("Location: " +  Home_inventory.myInventory[i].location, (int)
	(pf.getImageableX() + 25), y);
	y += dy;
	if ( Home_inventory.myInventory[i].marked)
	...
	g2D.drawString("Serial Number: " +
			 Home_inventory.myInventory[i].serialNumber, (int) (pf.getImageableX() + 25), y);
	y += dy;
	g2D.drawString("Price: $" +  Home_inventory.myInventory[i].purchasePrice + ",Purchased on: " +  Home_inventory.myInventory[i].purchaseDate, (int) (pf.getImageableX() +
	25), y);
	y += dy;
	g2D.drawString("Purchased at: " +Home_inventory.myInventory[i].purchaseLocation, (int) (pf.getImageableX() + 25), y);
	y += dy;
	g2D.drawString("Note: " + Home_inventory.myInventory[i].note, (int)
	(pf.getImageableX() + 25), y);
	y += dy;
	try
	{
	// maintain original width/height ratio
	Image inventoryImage = new ImageIcon(Home_inventory.myInventory[i].photoFile).getImage();
